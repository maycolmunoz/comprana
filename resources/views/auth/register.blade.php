<x-guest-layout>
	...
	<form method="POST" action="{{ route('register') }}" class="space-y-6">
		...
		<x-mary-input label="Nombre Completo" name="name" type="text" icon="o-user" value="{{ old('name') }}"
		...
		<x-mary-input label="Email" name="email" type="email" icon="o-envelope" value="{{ old('email') }}"
		...
		<x-mary-input label="Contraseña" name="password" type="password" icon="o-key" required
			autocomplete="new-password" class="bg-base-content/5 border-none focus:ring-red-600/20" />
		<x-base.input-error :messages="$errors->get('password')" class="" />
		...
		<x-base.input-error :messages="$errors->get('password_confirmation')" class="" />
		...
		<div class="text-center pt-2">
			<span
				class="text-[10px] font-bold uppercase tracking-widest text-base-content/30">¿Ya tienes cuenta?</span>
			<a href="{{ route('login') }}"
