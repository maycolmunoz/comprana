<x-guest-layout>
	...
	<form method="POST" action="{{ route('login') }}" class="space-y-6">
		...
		<div class="flex items-center justify-between">
			<label for="remember_me" class="inline-flex items-center group cursor-pointer">
				<input id="remember_me" type="checkbox"
					...
					<span
						class="ms-2 text-xs font-bold uppercase tracking-widest text-base-content/40 group-hover:text-red-600 transition-colors">Recordarme</span>
			</label>
		...
		<div class="pt-4">
			<x-mary-button type="submit" label="Iniciar Sesión"
				class="btn-primary w-full font-black text-sm uppercase tracking-widest shadow-xl shadow-red-600/20 rounded-2xl" />
		</div>

		<div class="text-center pt-2">
			<span
				class="text-[10px] font-bold uppercase tracking-widest text-base-content/30">¿No tienes cuenta?</span>
