...
<div class="container px-6 mx-auto animate__animated animate__fadeIn">
    ...
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div class="space-y-2">
            ...
            <h3
                class="text-3xl lg:text-4xl font-black tracking-tighter uppercase text-base-content flex items-center gap-3">
                @if ($sectionNull)
            ...
            @if (!$searchNull)
                <div
                    class="inline-flex items-center gap-2 px-3 py-1 bg-base-content/5 border border-base-content/5 rounded-full">
            ...
            @if (!$searchNull || !$sectionNull)
                <div class="flex items-center bg-base-100 p-1 rounded-2xl border border-base-content/5 shadow-sm">
                    ...
                    <div class="flex gap-1">
                        <x-mary-button wire:click="$set('order', 'asc')" :disabled="$orderAsc" label="Menor"
                            icon="o-bars-arrow-up"
                            class="btn-sm rounded-xl font-bold {{ $orderAsc ? 'btn-primary' : 'btn-ghost opacity-50' }}" />

                        <x-mary-button wire:click="$set('order', 'desc')" :disabled="$orderDesc" label="Mayor"
